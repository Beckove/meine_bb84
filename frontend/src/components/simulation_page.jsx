...
export default function SimulationPage() {
  ...
  const [result] = useState(state?.result || null);
  const [inputParams] = useState(state?.inputParams || null);

  useEffect(() => {
    if (!result || !inputParams) navigate('/setting_page');
  }, [result, inputParams, navigate]);

  //if (!result) return <div className="bg-gradient-to-br from-gray-900 via-purple-900 to-black flex items-center justify-center h-screen text-amber-500 text-6xl">Loading...</div>;

  const { alice_bits, bob_bits, alice_bases, bob_bases, sifted_key, quantum_bit_error_rate, matching_bases_count } = result;
  ...
  useLayoutEffect(() => { if (channelRef.current) setChannelWidth(channelRef.current.clientWidth); }, []);
  useEffect(() => {
    if (currentIdx < bitCount) {
      const t = setTimeout(() => setCurrentIdx(i => i + 1), 2000);
      return () => clearTimeout(t);
    } else if (currentIdx === bitCount) {
      setShowABC(true);
      const t2 = setTimeout(() => { setShowABC(false); setCurrentIdx(0); }, 4000);
      return () => clearTimeout(t2);
    }
  }, [currentIdx, bitCount]);

  const getBaseImage = base => base === 0 ? rect_base : base === 1 ? diagonal_base : Nonee;
  const getBitImage = (base, bit) => base === 0
    ? (bit === 0 ? hor_bit : ver_bit)
    : base === 1
      ? (bit === 0 ? diagonal1_bit : diagonal2_bit)
      : Nonee;
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="flex justify-between mb-10">
            ...
            <div className="flex flex-col items-center w-64">
              ...
              <div className="w-full h-72 overflow-y-auto border border-amber-600 rounded-2xl p-4 bg-gray-800 bg-opacity-40">
                ...
                {alice_bases.map((base, i) => (
                  <div key={i} className={`grid grid-cols-3 items-center gap-2 mb-3 py-2 px-1 rounded-lg ${currentIdx === i ? 'bg-amber-700 bg-opacity-30 font-bold' : ''}`}>
            ...
            <div className="flex flex-col items-center px-6">
              ...
              <img src={Nonee} alt="None" className="h-80 opacity-70" />
            </div>
            {/* Bob Table */}
            <div className="flex flex-col items-center w-64">
              ...
              <div className="w-full h-72 overflow-y-auto border border-amber-600 rounded-2xl p-4 bg-gray-800 bg-opacity-40">
                ...
                {bob_bases.map((base, i) => (
                  <div key={i} className={`grid grid-cols-3 items-center gap-2 mb-3 py-2 px-1 rounded-lg ${currentIdx === i ? 'bg-amber-700 bg-opacity-30 font-bold' : ''}`}>
                    ...
                    <span className="text-amber-100 text-2xl">{bob_bits[i] !== null ? bob_bits[i] : 'None'}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="mb-10">
            ...
            <div className="flex flex-col items-center">
              ...
              {/* Channel Bar */}
              <div ref={channelRef} className="w-full h-16.5 relative overflow-hidden rounded-lg border border-amber-600 bg-gray-800 bg-opacity-30">
          ...
          <div className="w-full border border-amber-600 rounded-2xl p-6 bg-gray-800 bg-opacity-40 text-amber-200 space-y-2 mb-10">
            ...
            <div><strong>Bob bits:</strong> {bob_bits.map(b => b === null ? 'None' : b).join(' ')}</div>
          ...
          <div className="flex justify-center items-start gap-12 mb-10">
            ...
            <ParameterPanel params={inputParams} onChange={() => {}} onReset={() => {}} className="bg-gray-800 bg-opacity-60 rounded-2xl p-4 border border-amber-600" />
          </div>
