...
def bb84_no_Eve(n_bits=1000, params=None):
    ...
    for bit, a_basis, b_basis in zip(alice_bits, alice_bases, bob_bases):
        ...
        qc_channel = transmit(qc, params['fiber_length'], params['fiber_loss'], params['source_efficiency'],
                              params['detector_efficiency'], params['perturb_probability'], params['sigma_sop'])
    ...
    sifted_key = []
    for a, b, bit, res in raw_key:
        if res is not None and a == b and bit == res:
            sifted_key.append(str(bit))

    qber = calculate_qber(alice_bits, np.array(bob_bits), alice_bases, bob_bases)

    return alice_bits.tolist(),np.array(bob_bits).tolist(), alice_bases.tolist(), bob_bases.tolist(), sifted_key, qber, matching_bases_count
...
def bb84_Eve(n_bits=1000, params=None):
    ...
    for bit, a_basis, eve_basis, b_basis in zip(alice_bits, alice_bases, eve_bases, bob_bases):
        qc = prepare_qubit(bit, a_basis)
        qc_channel = transmit(qc, params['fiber_length'], params['fiber_loss'],
