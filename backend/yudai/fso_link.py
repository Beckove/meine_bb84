...
class AtmosphericChannel:
    def __init__(self, lamda_nm=850, a=0.75, tau_zen=0.8, theta=10e-6,
        ...
        self.v = v
        self.A = A
        self.H_source = H_source  # chiều cao nguồn (m) - ví dụ vệ tinh

        self.muy_x = muy_x if muy_x is not None else 3 * a
        self.muy_y = muy_y if muy_y is not None else 2 * a
        self.sigma_x = sigma_x if sigma_x is not None else 2 * a
        self.sigma_y = sigma_y if sigma_y is not None else 3 * a
